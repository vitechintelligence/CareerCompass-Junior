...
export default async function BookOverviewPage({ params, searchParams }: { params: Promise<{ bookCode: string }>; searchParams: Promise<{ lang?: string; enrollmentId?: string }> }) {
  ...
  return (
    <main className="lessonApp">
      <header className="topbar lessonTopbar">
        ...
        <div className="lessonHeaderActions"><Link className="pill" href={`/learn/${book.code}?lang=${locale === "vi" ? "en" : "vi"}${enrollmentId ? `&enrollmentId=${encodeURIComponent(enrollmentId)}` : ""}`}>{locale === "vi" ? "English" : "Tiếng Việt"}</Link></div>
      </header>
          ...
          <div className="workspaceGrid">
            {book.units.map((unit) => (
              <article className="panel" key={unit.code}>
                ...
                <div className="actions"><Link className="button primary" href={`/learn/${book.code}/${unit.code}?lang=${locale}${enrollmentId ? `&enrollmentId=${encodeURIComponent(enrollmentId)}` : ""}`}>{locale === "vi" ? "Mở bài tương tác" : "Open interactive unit"}</Link></div>
              </article>
            ))}
          </div>
        </section>
